...
private:
    ...
    mutex m_conditional_mutex; // 线程休眠锁互斥变量
    condition_variable m_conditional_lock; // 线程环境锁，可以让线程处于休眠或者唤醒状态
public:
    // 线程池构造函数
    ...
    auto submit(F &&f, Args &&...args) -> future<decltype(f(args...))>{
        ...
        function<decltype(f(args...))()> func = bind(forward<F>(f), forward<Args>(args)...); // 连接函数和参数定义，特殊函数类型，避免左右值错误
